...
// region growing with seed point moves 

public class RegionGrowSeedPointMovesTest {
  public static void main(String[] args) throws Exception {
    long start = System.currentTimeMillis();

    String name1 = "C:/Users/PangTouXian/Desktop/paper picture & data/shopping mall/EM001_250000.txt";
    ...
    double d_threshold = 0.025f;
    double PFThreshold = 1;
    
    PointCloud pc1 = IOFunctions.readTextXYZ(name1);

    PointCloud pc_all_planes = new PointCloud(0);
    Colour cc = new Colour(pc_all_planes.getNumPoints(), 255, 255, 255);
    pc_all_planes.setColours(cc);

    PointCloud pc_all_planes_colour = new PointCloud(0);
    Colour c0 = new Colour(pc_all_planes_colour.getNumPoints(), 255, 255, 255);
    pc_all_planes_colour.setColours(c0);
    c0 = null;

    ArrayList<Integer> n_index = new ArrayList<Integer>();
    ArrayList<Integer> n_index0 = new ArrayList<Integer>();
    ...
    PlaneFactorCompute2 pfc = new PlaneFactorCompute2();
    ...
    do {
      ...
      System.out.println("rest points need to be tested:" + pc1.getNumPoints());
      ...
      // find a seed point
      do {
        double angle = ac.process(i,sizeThreshold);
        if (angle < angleThreshold && angle >= 0) {
          ...
          i++;
      } while (test && i < pc1.getNumPoints() - 1);

      if (i == pc1.getNumPoints() - 1) {
        stop_loop = 1;
      ...
      // find planar points
      PlaneFactorCompute2.setSharedReferences(snorm1, pc1.mCoordinates);
      n_index.addAll(ns);
      ...
      do {
        ...
        for (int l : ns) {
          ...
          for (int j : index1) {
            if (pointStatus[j]==1) 
              continue;
            double PF = pfc.process(i, n_index.get(m), j, d_threshold);
            if (PF >= PFThreshold) {
              n2.add(j);
              n_index0.add(l);
              pointStatus[j]=1;
      ...
      System.out.println("total planar points:" + pc_all_planes.getNumPoints());
